...
docker run -it --rm -v /etc/localtime:/etc/localtime -v /media/usb0/:/media/usb0/ -v /opt/dnas/secrets/redis_auth.py:/opt/dnas/dnas/dnas/redis_auth.py --name dnas_tmp dnas:latest /opt/pypy3.8-v7.3.7-aarch64/bin/pypy3 /opt/dnas/dnas/scripts/get_mrts.py --yesterday --update --enabled

# Parse any missing MRTs that were downloaded:
docker run -it --rm -v /etc/localtime:/etc/localtime -v /media/usb0/:/media/usb0/ -v /opt/dnas/secrets/redis_auth.py:/opt/dnas/dnas/dnas/redis_auth.py --name dnas_tmp dnas:latest /opt/pypy3.8-v7.3.7-aarch64/bin/pypy3 /opt/dnas/dnas/scripts/parse_mrts.py --yesterday --update --remove --enabled

# Generate the daily stats for yesterday:
docker run -it --rm -v /etc/localtime:/etc/localtime -v /media/usb0/:/media/usb0/ -v /opt/dnas/secrets/redis_auth.py:/opt/dnas/dnas/dnas/redis_auth.py --name dnas_tmp dnas:latest /opt/pypy3.8-v7.3.7-aarch64/bin/pypy3 /opt/dnas/dnas/scripts/stats.py --yesterday --update --enabled
