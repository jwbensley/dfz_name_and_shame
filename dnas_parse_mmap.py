...
def load_parse_mrt_rib(args):
    ...
    shm = shared_memory.SharedMemory(args[0])
    new_file = io.BytesIO(shm.buf[:])
    print(f"PID {os.getpid()} as file: {hasattr(new_file, 'read')}")
    ...
    rib_data = []
    entries = mrtparse.Reader(new_file)
...
def main():
    ...
    ###input(f"File size is {file_size/1024/1024}MBs")
    ...
    #rib_stats_chunks = multiprocessing.Pool.map(parse_mrt_update, chunks)
    #rib_stats_chunks = multiprocessing.Pool.map(parse_mrt_rib, chunks)
    chunks = []
    for i in range(num_procs):
        chunks.append([shm.name, i, num_procs])
